...
namespace UnityEngine.UI
{
    ...
    public class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        System.Collections.Generic.Stack<RectTransform> itemStack = new  System.Collections.Generic.Stack<RectTransform>();

        private Transform objpool = null;
        //缓存池最大缓存数量 超过则销毁
        private int stackCount = 5;
        //获取一个对象
        public virtual RectTransform GetItem()
        {
            RectTransform ii;
            ...
            {
                ii = Instantiate(item).transform as RectTransform;
            }
            return ii;
        }
        public virtual void ReMoveItem(RectTransform ii)
        {
            if(itemStack.Count > stackCount)
            {
                DestroyImmediate(ii.gameObject);
            }else
            {
                itemStack.Push(ii);
                ii.gameObject.SetActive(false);
                ii.SetParent(objpool,false);
        ...
        {
//            if (horizontal)
//                return item.rect.width + contentSpacing;
//            else
//                return item.rect.height + contentSpacing;
        ...
        protected LayoutGroup m_LayoutGroup = null;
        protected float contentSpacing
        {
            get
            {
                ...
                if (content != null)
                {
                    ...
                    VerticalLayoutGroup layout2 = content.GetComponent<VerticalLayoutGroup>();
                    if (layout2 != null)
                    {
                        m_ContentSpacing = layout2.spacing;
                    }
        ...
        public bool horizontal { get { return m_Horizontal; } set { m_Horizontal = value;  m_Vertical = !value;} }

        [SerializeField]
        private bool m_Vertical = true;
        public bool vertical { get { return m_Vertical; } set { m_Vertical = value; m_Horizontal = !value; } }
        ...
        protected override void Awake()
        {
            ...
            objpool.gameObject.hideFlags = HideFlags.HideInHierarchy;
...
//            go.GetComponent<ItemInterface>().SetData()
            ReMoveItem(go.GetComponent<RectTransform>());
        }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                //{
                //    RefillCells(content.GetChild(0).GetComponent<ItemInterface>().index);
                //}
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    {
//                        item.SetData(content.GetChild(i), itemTypeEnd);
                        content.GetChild(i).GetComponent<ItemInterface>().SetData(itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        //这个函数是为了倒序能够不频发刷新每个item,所以调用一下
        public void RefreshNewItem()
        {
            //if (this.normalizedPosition.x < 0.01f)
            //{
            //    return;
            //}
            //先把顺序的新创建元素放到首位
            if (content.childCount > 0)
            {
                ...
                {
                    content.GetChild(0).GetComponent<ItemInterface>().SetData(content.GetChild(1).GetComponent<ItemInterface>().index);
                }
                else
                {
                    content.GetChild(0).GetComponent<ItemInterface>().SetData(0);
                }
                
            }
            //然后修改一下index,其实此时的数据是刚刚好正确的
            itemTypeEnd = itemTypeStart;
            for (int i = 0; i < content.childCount; i++)
            {
                if (itemTypeEnd < totalCount)
                {
                    //                        item.SetData(content.GetChild(i), itemTypeEnd);
                    content.GetChild(i).GetComponent<ItemInterface>().index = itemTypeEnd;//.SetData(itemTypeEnd);
                    itemTypeEnd++;
                }
                else
                {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            if (!Application.isPlaying || totalCount < 0 || contentConstraintCount > 1 || item == null)
                return;
        ...
        {            
            RectTransform nextItem = GetItem();//Instantiate(item.gameObject).transform as RectTransform;
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            nextItem.GetComponent<ItemInterface>().SetData(itemIdx);
            return nextItem;
        }
        //==========LoopScrollRect==========
